...
function CMusicPlayer:StopAll()
    if not gAppliactionBackground  then
        self.m_BgPath = nil
...
function CMusicPlayer:PlayEffectById(nEffectId, bLoop, nDelay)
    ...
    if strPath ~= '' then
        -- 播放
        _effectInstanceId = self:PlayEffect(strPath, bLoop)
        -- 同类型音效过滤
...
function CMusicPlayer:PlayEffect(strPath, bLoop, nDelay)
    if not gAppliactionBackground then
        print("=====play effect music path: ===", strPath)
        ...
        print("=====play effect music full path: ===", strPath, _instanceId)
        return _instanceId
    end
end
...
function CMusicPlayer:AddPreLoadEffect(strName)
    strName = cc.FileUtils:getInstance():fullPathForFilename(strName)
...
--[[
 ...
 -- 备    注:
 --]] 
function CMusicPlayer:UnloadFightEffect(strName)
    strName = cc.FileUtils:getInstance():fullPathForFilename(strName)
...
--[[
 ...
 -- 备    注:
 --]] 
function CMusicPlayer:UnloadOneEffect( strName )
    strName = cc.FileUtils:getInstance():fullPathForFilename(strName)
    self.m_Engine:unloadEffect(strName)
    self.m_PreLoadList[strName] = false
end
