...
local ipairs = ipairs
local string = string
local release_print = release_print
local io = io
...
function register_file(filename)
    ...
    if not IsGameServer then
        --local buffer = cc.FileUtils:getInstance():getFileDataToPb(filename)
        local buffer = cc.FileUtils:getInstance():getStringFromFile(filename)
        if not buffer then
            log_error("protobuf.log", "open file failed:%s", filename)
            return
        end
        c._env_register(P, buffer)
    else
