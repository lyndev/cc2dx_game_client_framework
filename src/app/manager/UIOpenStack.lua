...
function CUIOpenStack:OpenTopStackUI()
    ...
    if topUI then
        ...
        -- UI打开
        local _uiCtrl = CtrlManager.GetCtrl(topUI.name)
        if _uiCtrl then
            _uiCtrl:Awake()
        end
    else
...
function CUIOpenStack:CloseTopStackUI()
    ...
    local topUI = self.stack.uiList[self.stack.nTop]
    if topUI then
        panelMgr:ClosePanel(topUI.name)
    else
        log_error(LOG_FILE_NAME, "stack top ui is empty")
    end
end
...
function CUIOpenStack:SetTopStackUIHIDE()
    ...
    local topUI = self.stack.uiList[self.stack.nTop]
    if topUI then
        local _topUICtrl = CtrlManager.GetCtrl(topUI.name)
        if _topUICtrl and _topUICtrl["SetVisible"] then
            _topUICtrl:SetVisible(false)
        end
    else
        log_error(LOG_FILE_NAME, "stack top ui is empty")
    end
...
function CUIOpenStack:RecordUIState()
    for k, v in pairs(self.stack.uiList) do
        if v and v.name then
            local _uiCtrl = CtrlManager.GetCtrl(v.name)
            if _uiCtrl then
                local _bShow = _uiCtrl.GetUIRoot():GetActive()
...
function CUIOpenStack:RecoverRecordUIState()
    for k, v in pairs(self.m_tRecordUIState) do
        if k then
            local _uiCtrl = CtrlManager.GetCtrl(k)
            if _uiCtrl then
               _uiCtrl.GetUIRoot():SetActive(v)                   
            end
        end
    end
